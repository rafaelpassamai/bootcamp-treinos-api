...
export function Chat({ embedded = false, initialMessage }: ChatProps) {
  ...
  const errorMessage =
    ...
    error?.message.includes("GEMINI_NOT_CONFIGURED")
      ? "O chat está indisponível: configure a GEMINI_API_KEY no backend."
      : error?.message;
  ...
  const chatContent = (
    ...
    >
      ...
      <div className="flex shrink-0 flex-col gap-3">
        {errorMessage && (
          <p className="px-5 font-heading text-xs text-red-500">{errorMessage}</p>
        )}
